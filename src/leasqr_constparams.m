function [f,p,kvg,iter,corp,covp,covr,stdresid,Z,r2]= ...
      leasqr(x,y,pin,F,stol,niter,wt,dp,dFdp,const_params,options)
%function[f,p,kvg,iter,corp,covp,covr,stdresid,Z,r2]=
%                   leasqr(x,y,pin,F,{stol,niter,wt,dp,dFdp,options,const_params})
%
% Version 3.beta
%  {}= optional parameters
...
%     default = Inf*ones().
% const_params=parameters passed to F and dFdp that are NOT being fitted - (SMP, 5/6/2004)
%
%          OUTPUT VARIABLES
...
y=y(:); wt=wt(:); pin=pin(:); dp=dp(:); const_params=const_params(:);%change all vectors to columns
...
% SMP Changed this to 10 b/c OPTIONS arg was pushed back one by const_params
if (nargin <= 10), 
...
f=feval(F,x,p,const_params); fbest=f; pbest=p;
r=wt.*(y-f);
sbest=r'*r;
nrm=zeros(n,1);
...
for iter=1:niter,
  pprev=pbest;
  prt=feval(dFdp,x,fbest,pprev,dp,F,const_params);
  r=wt.*(y-fbest);
  sprev=sbest;
  sgoal=(1-stol)*sprev;
  for j=1:n,
      %fprintf('n==%d, j==%d, size(dp)=(%d,%d)\n', n, j, size(dp,1), size(dp,2));
    if dp(j)==0,
      nrm(j)=0;
    else
      prt(:,j)=wt.*prt(:,j);
...
jac=feval(dFdp,x,f,p,dp,F,const_params);
msk = dp ~= 0;
